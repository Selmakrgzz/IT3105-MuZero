...
class NeuralNetworkManager(nn.Module):
    ...
    def loss(self, batch):
        ...
        # unpack states and actions
        states = batch["states"].to(self.device).float()
        actions = batch["actions"].to(self.device).long()
        print(actions)
        # targets unpack
        
        target_policy = batch["target_policy"].to(self.device).float()
        target_value = batch["target_value"].to(self.device).float()
        target_reward = batch["target_reward"].to(self.device).float()

        
        pred = self.unroll(states, actions)
        
        
        # soft-target cross entropy for MCTS policy targets
